...
const ContractInteraction: React.FC<Props> = ({ abi, contractAddress, web3, account }) => {
  ...
  const functions = abi.filter(item => item.type === 'function') as ContractFunction[];
  ...
  const formatParameter = (value: string, type: string) => {
    try {
      if (type.includes('[]')) {
  ...
  const validateInputs = () => {
    ...
    try {
      selectedFunction.inputs.forEach(input => {
        ...
        formatParameter(value, input.type);
      });
      return true;
    } catch (error: any) {
  ...
  const executeFunction = async () => {
    ...
    try {
      const params = selectedFunction.inputs.map(input => 
        formatParameter(inputValues[input.name], input.type)
      );
      ...
      if (selectedFunction.stateMutability === 'view' || selectedFunction.stateMutability === 'pure') {
        const result = await contract.methods[selectedFunction.name](...params).call();
        ...
        const gasEstimate = await contract.methods[selectedFunction.name](...params)
          .estimateGas({ from: account });
        
        const tx = await contract.methods[selectedFunction.name](...params)
          .send({ 
            from: account,
                ...
                {selectedFunction.inputs.map((input) => (
                  <div key={input.name} className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">
                      {input.type.includes('[]') && 'Format: ["item1", "item2"] or [1, 2, 3]'}
                    </p>
                  </div>
                ))}
                <button
